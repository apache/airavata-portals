...
def get_notifications(request: AiravataRequest) -> dict[str, Any]:
    if request.user.is_authenticated and hasattr(request, "airavata_client"):
        ...
        for notification in notifications:
            ...
            publishedTime = datetime.datetime.fromtimestamp(notification.publishedTime / 1000)
        ...
        return {"notifications": json.dumps(valid_notifications), "unread_notifications": unread_notifications}
    else:
...
def _get_app_nav(request: HttpRequest, current_app: AiravataAppConfig) -> list[dict[str, Any]]:
    if hasattr(current_app, "nav"):
        ...
        nav = [
            {"label": current_app.verbose_name, "icon": "fa " + current_app.fa_icon_class, "url": current_app.url_home}
        ]
    return nav
