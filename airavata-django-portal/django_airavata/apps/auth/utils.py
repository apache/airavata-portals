...
def get_service_account_authz_token():
    ...
    token = oauth.fetch_token(token_url=token_url, client_id=client_id, client_secret=client_secret, verify=verify)
...
def send_new_user_email(request, username, email, first_name, last_name):
    ...
    new_user_email_template = models.EmailTemplate.objects.get(pk=models.NEW_USER_EMAIL_TEMPLATE)
...
def send_admin_user_completed_profile(request, user_profile):
    ...
    extended_profile_values = (
        user_profile.extended_profile_values.filter(ext_user_profile_field__deleted=False)
        .order_by("ext_user_profile_field__order")
        .all()
    )
    context = Context(
        {
    ...
    user_profile_completed_template = models.EmailTemplate.objects.get(pk=models.USER_PROFILE_COMPLETED_TEMPLATE)
...
def send_email_to_admins(subject, body):
    msg = EmailMessage(
        ...
        to=[f'"{a[0]}" <{a[1]}>' for a in getattr(settings, "PORTAL_ADMINS", settings.ADMINS)],
    )
...
def send_email_to_user(template_id, context):
    ...
    msg = EmailMessage(
        ...
        body=body,
        from_email=f'"{settings.PORTAL_TITLE}" <{settings.SERVER_EMAIL}>',
        to=['"{} {}" <{}>'.format(context["first_name"], context["last_name"], context["email"])],
        reply_to=[f'"{a[0]}" <{a[1]}>' for a in getattr(settings, "PORTAL_ADMINS", settings.ADMINS)],
    )
    msg.content_subtype = "html"
    msg.send()
