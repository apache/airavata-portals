...
def start_username_password_login(request):
    ...
    return render(
        ...
        {"next": request.GET.get("next", None), "options": settings.AUTHENTICATION_OPTIONS, "login_type": "password"},
    )
...
def handle_login(request):
    ...
    try:
        ...
        messages.error(request, f"Login failed: {str(err)}. Please try again.")
    if login_desktop:
...
def callback_error(request, idp_alias):
    ...
    return render(
        request,
        "django_airavata_auth/callback-error.html",
        {
            "idp_alias": idp_alias,
            "options": options,
        },
...
def create_account(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                success = iam_admin_client.register_user(username, email, first_name, last_name, password)
                if not success:
                    form.add_error(None, ValidationError("Failed to register user with IAM service"))
                else:
    ...
    return render(
        request, "django_airavata_auth/create_account.html", {"options": settings.AUTHENTICATION_OPTIONS, "form": form}
    )
...
def verify_email(request, code):
    ...
    try:
        ...
        if iam_admin_client.is_user_enabled(username):
            ...
            messages.success(request, "Your account has already been successfully created. Please log in now.")
            ...
            utils.send_new_user_email(request, username, email_address, first_name, last_name)
            messages.success(request, "Your account has been successfully created. Please log in now.")
            return redirect(login_url)
    except ObjectDoesNotExist:
        # if doesn't exist, give user a form where they can enter their
        # username to resend verification code
        logger.exception(f"EmailVerification object doesn't exist for code {code}", extra={"request": request})
        messages.error(
            request,
        ...
        messages.error(
            request, "Email verification failed. Please try clicking the email verification link again later."
        )
        return redirect(reverse("django_airavata_auth:create_account"))
            ...
            try:
                ...
                if iam_admin_client.is_user_exist(username):
                    ...
                    _create_and_send_email_verification_link(
                        request, username, email_address, user_profile.firstName, user_profile.lastName
                    )
                    messages.success(
                        request,
                ...
                logger.exception("Failed to resend email verification link", exc_info=e, extra={"request": request})
                form.add_error(None, ValidationError(str(e)))
    else:
        form = forms.ResendEmailVerificationLinkForm()
    return render(request, "django_airavata_auth/verify_email.html", {"form": form})


def _create_and_send_email_verification_link(request, username, email, first_name, last_name, next=None):

    email_verification = models.EmailVerification(username=username, next=next)
    email_verification.save()

    verification_uri = request.build_absolute_uri(
        reverse("django_airavata_auth:verify_email", kwargs={"code": email_verification.verification_code})
    )
    logger.debug(f"verification_uri={verification_uri}")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                logger.exception(
                    "Failed to generate password reset request for user", exc_info=e, extra={"request": request}
                )
                form.add_error(None, ValidationError(str(e)))
    else:
        form = forms.ForgotPasswordForm()
    return render(request, "django_airavata_auth/forgot_password.html", {"form": form})
...
def _create_and_send_password_reset_request_link(request, username):
    ...
    verification_uri = request.build_absolute_uri(
        reverse("django_airavata_auth:reset_password", kwargs={"code": password_reset_request.reset_code})
    )
    logger.debug(f"password reset verification_uri={verification_uri}")
...
def reset_password(request, code):
    try:
        password_reset_request = models.PasswordResetRequest.objects.get(reset_code=code)
    except ObjectDoesNotExist:
        messages.error(request, "Reset password link is invalid. Please try again.")
        return redirect(reverse("django_airavata_auth:forgot_password"))

    now = datetime.now(UTC)
    if now - password_reset_request.created_date > timedelta(days=1):
        password_reset_request.delete()
        messages.error(request, "Reset password link has expired. Please try again.")
        return redirect(reverse("django_airavata_auth:forgot_password"))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                password = form.cleaned_data["password"]
                success = iam_admin_client.reset_user_password(password_reset_request.username, password)
                if not success:
                    messages.error(request, "Failed to reset password. Please try again.")
                    return redirect(reverse("django_airavata_auth:forgot_password"))
                else:
                    password_reset_request.delete()
                    messages.success(request, "You may now log in with your new password.")
                    return redirect(reverse("django_airavata_auth:login_with_password"))
            except Exception as e:
                logger.exception("Failed to reset password for user", exc_info=e, extra={"request": request})
                form.add_error(None, ValidationError(str(e)))
    else:
        form = forms.ResetPasswordForm()
    return render(request, "django_airavata_auth/reset_password.html", {"form": form, "code": code})
...
def _create_login_desktop_success_response(request, download_code=False, show_code=False):
    ...
    return redirect(reverse("django_airavata_auth:login_desktop_success") + "?" + urlencode(query_params))


def _create_login_desktop_failed_response(request, idp_alias=None):
    params = {"status": "failed"}
    if idp_alias is not None:
        return redirect(reverse("django_airavata_auth:callback-error", args=(idp_alias,)) + "?" + urlencode(params))
    if "username" in request.POST:
        params["username"] = request.POST["username"]
    return redirect(reverse("django_airavata_auth:login_desktop") + "?" + urlencode(params))


@login_required
def access_token_redirect(request):
    redirect_uri = request.GET["redirect_uri"]
    config = next(filter(lambda d: d.get("URI") == redirect_uri, settings.ACCESS_TOKEN_REDIRECT_ALLOWED_URIS), None)
    if config is None:
        logger.warning(
            f"redirect_uri value '{redirect_uri}' is not configured in ACCESS_TOKEN_REDIRECT_ALLOWED_URIS setting"
        )
        return HttpResponseForbidden("Invalid redirect_uri value")
    return redirect(
        redirect_uri + f"{'&' if '?' in redirect_uri else '?'}{config.get('PARAM_NAME', 'access_token')}="
...
def user_profile(request):
    return render(
        request,
        "django_airavata_auth/base.html",
        {
...
class IsUserOrReadOnlyForAdmins(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS and request.is_gateway_admin:
            return True
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def verify_email_change(self, request, pk=None):
        ...
        try:
            ...
            if user_profile_client.doesUserExist(request.authz_token, request.user.username, settings.GATEWAY_ID):
            ...
            # otherwise, update the user's email in the Keycloak user store
                ...
                iam_admin_client.update_user(request.user.username, email=pending_email_change.email_address)
        except Exception as e:
...
class ExtendedUserProfileValueViewset(
