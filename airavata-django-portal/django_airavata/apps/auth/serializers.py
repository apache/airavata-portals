...
class UserSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        if instance.email != validated_data["email"]:
            ...
            pending_email_change = models.PendingEmailChange.objects.create(
                user=request.user, email_address=validated_data["email"]
            )
        ...
        # otherwise, update in Keycloak user store
            ...
            iam_admin_client.update_user(
                request.user.username, first_name=instance.first_name, last_name=instance.last_name
            )
        return instance
...
class ExtendedUserProfileFieldSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.field_type == "single_choice":
            ...
            result["choices"] = ExtendedUserProfileFieldChoiceSerializer(
                instance.single_choice.choices.filter(deleted=False).order_by("order"), many=True
            ).data
        if instance.field_type == "multi_choice":
            result["other"] = instance.multi_choice.other
            result["choices"] = ExtendedUserProfileFieldChoiceSerializer(
                instance.multi_choice.choices.filter(deleted=False).order_by("order"), many=True
            ).data
    ...
    def create(self, validated_data):
        ...
        if field_type == "text":
            ...
            instance = models.ExtendedUserProfileAgreementField.objects.create(
                **validated_data, checkbox_label=checkbox_label
            )
        else:
    ...
    def update(self, instance, validated_data):
        ...
        if instance.field_type == "single_choice":
            ...
            instance.user_agreement.checkbox_label = validated_data.pop(
                "checkbox_label", instance.user_agreement.checkbox_label
            )
    ...
    def create(self, validated_data):
        ...
        if ext_user_profile_field.field_type == "text":
            ...
            return models.ExtendedUserProfileTextValue.objects.create(
                ext_user_profile_field=ext_user_profile_field, user_profile=user_profile, text_value=text_value
            )
            ...
            for choice in choices:
                models.ExtendedUserProfileMultiChoiceValueChoice.objects.create(value=choice, multi_choice_value=value)
            return value
        ...
        if ext_user_profile_field.field_type == "single_choice":
            ...
            if len(choices) == 1 and other_value != "":
                raise serializers.ValidationError(
                    "Must specify only a single choice or the other choice, but not both."
                )
            if len(choices) == 0 and other_value == "":
                raise serializers.ValidationError(
                    "Must specify one of a single choice or the other choice (but not both)."
                )
        elif ext_user_profile_field.field_type == "multi_choice":
            choices = attrs.get("choices", [])
            other_value = attrs.get("other_value", "")
