...
class HandleLoginViewTestCase(TestCase):
    def test_with_get_request(self):
        ...
        response = self.client.get(reverse("django_airavata_auth:handle_login"))
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response["Location"], reverse("django_airavata_auth:login"))
...
class CreateAccountViewTestCase(TestCase):
    ...
    def test_submit_create_account_with_next(self, forms_iam_admin_client, views_iam_admin_client):
        ...
        # 'messages' framework
        response = SessionMiddleware(MessageMiddleware(views.create_account))(request)
        self.assertEqual(302, response.status_code)
        self.assertIsInstance(response, HttpResponseRedirect)
        self.assertEqual(reverse("django_airavata_auth:create_account"), response.url)
        self.assertEqual(len(mail.outbox), 1)
        email_verification = models.EmailVerification.objects.get(username=data["username"])
    ...
    def test_submit_invalid_create_account_with_next(self, forms_iam_admin_client):
        ...
        self.assertRegex(response.content.decode("utf-8"), '(?s)type="hidden"[^>]+name="next"[^>]+value="/next/path"')
...
class VerifyEmailViewTestCase(TestCase):
    ...
    def test_verify_email(self, views_iam_admin_client):
        ...
        verify_email_url = reverse(
            "django_airavata_auth:verify_email", kwargs={"code": email_verification.verification_code}
        )
        ...
        response = SessionMiddleware(
            MessageMiddleware(lambda r: views.verify_email(r, email_verification.verification_code))
        )(request)
        self.assertIsInstance(response, HttpResponseRedirect)
        self.assertEqual(reverse("django_airavata_auth:login"), response.url)
        email_verification = models.EmailVerification.objects.get(username="testuser")
    ...
    def test_verify_email_with_next(self, views_iam_admin_client):
        ...
        # create an EmailVerification record with a next url
        email_verification = models.EmailVerification(username="testuser", next="/next/path")
        email_verification.save()
        ...
        self.assertIsInstance(response, HttpResponseRedirect)
        self.assertEqual(
            reverse("django_airavata_auth:login") + "?" + urlencode({"next": email_verification.next}), response.url
        )
        email_verification = models.EmailVerification.objects.get(username="testuser")
        self.assertTrue(email_verification.verified)
...
class ResendEmailLinkTestCase(TestCase):
    ...
    def test_resend_email_link(self, views_iam_admin_client):
        ...
        # SessionMiddleware and MessageMiddleware since create_account uses
        # 'messages' framework
        response = SessionMiddleware(MessageMiddleware(lambda r: views.resend_email_link(r)))(request)
