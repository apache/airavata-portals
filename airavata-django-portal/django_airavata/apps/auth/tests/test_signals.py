...
@override_settings(
    GATEWAY_ID=GATEWAY_ID, SERVER_EMAIL=SERVER_EMAIL, PORTAL_TITLE=PORTAL_TITLE, PORTAL_ADMINS=PORTAL_ADMINS
)
class EmailUserAddedToGroupSignalReceiverTests(TestCase):
    ...
    def test(self):
        ...
        user_added_to_group.send(None, user=self.user, groups=[group], request=self.request)
        self.assertEqual(len(mail.outbox), 1)
        msg = mail.outbox[0]
        self._assert_common_email_attributes(msg)
        self.assertEqual(msg.subject, f"You've been added to group [{group.name}] in {PORTAL_TITLE}")

    def test_multiple_groups(self):
        group1 = GroupModel(id="abc123", name="Test Group")
        group2 = GroupModel(id="group2", name="Group 2")
        user_added_to_group.send(None, user=self.user, groups=[group1, group2], request=self.request)
        self.assertEqual(len(mail.outbox), 1)
        msg = mail.outbox[0]
        self._assert_common_email_attributes(msg)
        self.assertEqual(
            msg.subject, f"You've been added to groups [{group1.name}] and [{group2.name}] in {PORTAL_TITLE}"
        )
