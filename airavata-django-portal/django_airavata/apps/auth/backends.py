...
class KeycloakBackend:
    ...
    @sensitive_variables()
    def authenticate(self, request=None, username=None, password=None, refresh_token=None, idp_alias=None):
        try:
            user = None
            access_token = None
            ...
            # user is already logged in and can use refresh token
                ...
                token, userinfo = self._get_token_and_userinfo_from_refresh_token(request)
                ...
                token, userinfo = self._get_token_and_userinfo_from_refresh_token(request, refresh_token=refresh_token)
                if token is None:  # refresh failed
                    return None
                self._process_token(request, token)
                user = self._process_userinfo(request, userinfo)
    ...
    def _get_token_and_userinfo_password_flow(self, username, password):
        try:
            ...
            logger.warning(f"Failed to log in user {username} with password: {e}")
            return None, None
    ...
    def _get_token_and_userinfo_redirect_flow(self, request):
        ...
        oauth2_session = OAuth2Session(client_id, scope="openid profile email", redirect_uri=redirect_uri, state=state)
        verify = verify_ssl
        if verify_ssl and hasattr(settings, "KEYCLOAK_CA_CERTFILE"):
            verify = settings.KEYCLOAK_CA_CERTFILE
        ...
        token = oauth2_session.fetch_token(
            token_url, client_secret=client_secret, authorization_response=authorization_code_url, verify=verify
        )
        userinfo = oauth2_session.get(userinfo_url).json()
        return token, userinfo

    def _get_token_and_userinfo_from_refresh_token(self, request, refresh_token=None):
        client_id = settings.KEYCLOAK_CLIENT_ID
        client_secret = settings.KEYCLOAK_CLIENT_SECRET
        token_url = settings.KEYCLOAK_TOKEN_URL
        userinfo_url = settings.KEYCLOAK_USERINFO_URL
        ...
        if verify_ssl and hasattr(settings, "KEYCLOAK_CA_CERTFILE"):
            verify = settings.KEYCLOAK_CA_CERTFILE
        refresh_token_ = refresh_token if refresh_token is not None else request.session["REFRESH_TOKEN"]
        ...
        try:
            ...
            logger.warning(f"Failed to refresh token for user {request.user.username} : {e}")
            return None, None
    ...
    def _get_userinfo_from_token(self, request, token):
        ...
        userinfo_url = settings.KEYCLOAK_USERINFO_URL
        verify_ssl = settings.KEYCLOAK_VERIFY_SSL
        oauth2_session = OAuth2Session(client_id, token={"access_token": token})
        verify = verify_ssl
        if verify_ssl and hasattr(settings, "KEYCLOAK_CA_CERTFILE"):
            verify = settings.KEYCLOAK_CA_CERTFILE
        userinfo = oauth2_session.get(userinfo_url, verify=verify).json()
    ...
    def _get_or_create_user(self, sub, username):
        ...
        try:
            user_profile = models.UserProfile.objects.get(userinfo__claim="sub", userinfo__value=sub)
    ...
    def _store_idp_userinfo(self, user, token, idp_alias):
        try:
            ...
            if idp_token_url is None or userinfo_url is None:
                logger.debug(
                    f"idp_token_url and/or userinfo_url not set for {idp_alias} "
    ...
    def _check_username_initialization(self, request, user):
        ...
        user_profile = user.user_profile
        if (
            not user_profile.username_initialized
            and user_profile.userinfo_set.filter(claim="email").exists()
            and user_profile.userinfo_set.filter(claim="preferred_username").exists()
            and user_profile.userinfo_set.get(claim="email").value
            == user_profile.userinfo_set.get(claim="preferred_username").value
        ):
        ...
        if not user_profile.username_initialized and not user_profile.is_username_valid:
            try:
                ...
                logger.exception(
                    f"Failed to send alert about username being uninitialized: {user.username}",
