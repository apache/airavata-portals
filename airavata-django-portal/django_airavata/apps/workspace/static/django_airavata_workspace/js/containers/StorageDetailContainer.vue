<template>
  <div>
    ...
    <template v-else-if="resource">
      ...
      <div v-if="testStatus" class="alert mb-3"
        :class="testStatus.success ? 'alert-success' : 'alert-danger'"
        style="font-size:0.875rem;">
        <i :class="testStatus.success ? 'fa fa-check-circle' : 'fa fa-times-circle'" class="me-1"></i>
        {{ testStatus.message }}
        <button type="button" class="btn-close float-end" style="font-size:0.75rem;" @click="testStatus = null"></button>
      </div>
          ...
          <div class="row g-3">
            ...
            <div class="col-12">
              <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" id="enabledToggle" v-model="resource.enabled" />
    ...
    <div v-else class="alert alert-danger">
      Failed to load storage resource.
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    async testConnection() {
      ...
      try {
        ...
        this.testing = false;
        this.testStatus = { success: false, message: e?.message || "Failed to initiate connection test." };
      }
    },
    onSshResult(event) {
      this.testing = false;
