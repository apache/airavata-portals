...
class ProjectSerializer(proto_utils.create_serializer_class(Project)):
    class Meta:
        required = ("name",)
    ...
    experiments = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:project-experiments", lookup_field="projectID", lookup_url_kwarg="project_id"
    )
...
class ApplicationModuleSerializer(proto_utils.create_serializer_class(ApplicationModule)):
    url = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:application-detail", lookup_field="appModuleId", lookup_url_kwarg="app_module_id"
    )
...
class CommandObjectSerializer(proto_utils.create_serializer_class(CommandObject)):
    pass


class SetEnvPathsSerializer(proto_utils.create_serializer_class(SetEnvPaths)):
    pass


class ApplicationDeploymentDescriptionSerializer(proto_utils.create_serializer_class(ApplicationDeploymentDescription)):
    url = FullyEncodedHyperlinkedIdentityField(
    ...
    userHasWriteAccess = serializers.SerializerMethodField()
    moduleLoadCmds = OrderedListField(order_by="commandOrder", child=CommandObjectSerializer(), allow_null=True)
    preJobCommands = OrderedListField(order_by="commandOrder", child=CommandObjectSerializer(), allow_null=True)
    postJobCommands = OrderedListField(order_by="commandOrder", child=CommandObjectSerializer(), allow_null=True)
    libPrependPaths = OrderedListField(order_by="envPathOrder", child=SetEnvPathsSerializer(), allow_null=True)
    libAppendPaths = OrderedListField(order_by="envPathOrder", child=SetEnvPathsSerializer(), allow_null=True)
    setEnvironment = OrderedListField(order_by="envPathOrder", child=SetEnvPathsSerializer(), allow_null=True)
...
class ExperimentStatusSerializer(proto_utils.create_serializer_class(ExperimentStatus)):
    timeOfStateChange = UTCPosixTimestampDateTimeField()


class ProcessStatusSerializer(proto_utils.create_serializer_class(ProcessStatus)):
    timeOfStateChange = UTCPosixTimestampDateTimeField()


class ExperimentSerializer(proto_utils.create_serializer_class(ExperimentModel)):
    class Meta:
        required = ("projectId", "experimentType", "experimentName")
        read_only = ("userName", "gatewayId")

    url = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:experiment-detail", lookup_field="experimentId", lookup_url_kwarg="experiment_id"
    )
    ...
    jobs = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:experiment-jobs", lookup_field="experimentId", lookup_url_kwarg="experiment_id"
    )
    shared_entity = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:shared-entity-detail", lookup_field="experimentId", lookup_url_kwarg="entity_id"
    )
    experimentInputs = OrderedListField(order_by="inputOrder", child=InputDataObjectTypeSerializer(), allow_null=True)
    experimentOutputs = serializers.ListField(child=OutputDataObjectTypeSerializer(), allow_null=True)
    creationTime = UTCPosixTimestampDateTimeField(allow_null=True)
    ...
    def _add_intermediate_output_information(self, experiment, representation):
        ...
        # experiment outputs
        if experiment.experimentStatus and experiment.experimentStatus[-1].state == ExperimentState.EXECUTING:
...
class DataReplicaLocationSerializer(proto_utils.create_serializer_class(DataReplicaLocationModel)):
    creationTime = UTCPosixTimestampDateTimeField()
    lastModifiedTime = UTCPosixTimestampDateTimeField()


class DataProductSerializer(proto_utils.create_serializer_class(DataProductModel)):
    creationTime = UTCPosixTimestampDateTimeField()
...
class BaseExperimentSummarySerializer(proto_utils.create_serializer_class(ExperimentSummaryModel)):
    creationTime = UTCPosixTimestampDateTimeField()
...
class ComputeResourceReservationSerializer(proto_utils.create_serializer_class(ComputeResourceReservation)):
    startTime = UTCPosixTimestampDateTimeField(allow_null=True)
    endTime = UTCPosixTimestampDateTimeField(allow_null=True)


class GroupComputeResourcePreferenceSerializer(proto_utils.create_serializer_class(GroupComputeResourcePreference)):
    reservations = serializers.SerializerMethodField()

    # Check if the object (e.g. SLURM type) has the 'reservations' attribute
    def get_reservations(self, obj):
        if hasattr(obj, "reservations"):
            reservations_data = obj.reservations
                ...
                if resource_type == ResourceType.SLURM:
                    ...
                    if slurm_data and isinstance(slurm_data, dict) and len(slurm_data) > 0:
                        try:
                            ...
                            if (
                                "groupSSHAccountProvisionerConfigs" in slurm_data
                                and slurm_data["groupSSHAccountProvisionerConfigs"]
                            ):
    ...
    def create(self, validated_data):
            ...
            if not hasattr(validated_data, "resourceType") or validated_data.resourceType is None:
                ...
                if hasattr(validated_data, "specificPreferences") and validated_data.specificPreferences:
                    if isinstance(validated_data.specificPreferences, (dict, OrderedDict)):
                        ...
                        hasattr(validated_data.specificPreferences, "slurm")
        ...
        if resource_type is None:
            if "allocationProjectNumber" in data or (
                "specificPreferences" in data
                and isinstance(data.get("specificPreferences"), dict)
                and "slurm" in data.get("specificPreferences", {})
            ):
                resource_type = ResourceType.SLURM
            elif (
                "specificPreferences" in data
                and isinstance(data.get("specificPreferences"), dict)
                and "aws" in data.get("specificPreferences", {})
                or "region" in data
                or "preferredAmiId" in data
                or "preferredInstanceType" in data
            ):
                resource_type = ResourceType.AWS
            else:
                resource_type = ResourceType.SLURM
        ...
        if specific_prefs is None:
                ...
                if union_type_class:
                    ...
                    try:
                        ...
                        if test_instance.specificPreferences is not None:
                            ...
                            log.warning("GCPreference create: Could not create union type, instance may be invalid")
                    except Exception as e:
                        log.error(
            ...
            if "slurm" in specific_prefs:
                ...
                if (
                    "groupSSHAccountProvisionerConfigs" in slurm_dict
                    and slurm_dict["groupSSHAccountProvisionerConfigs"]
                ):
        ...
        if not hasattr(instance, "resourceType") or instance.resourceType is None:
            if hasattr(instance, "specificPreferences") and instance.specificPreferences:
                ...
                if isinstance(instance.specificPreferences, (dict, OrderedDict)):
                    if (
                        "slurm" in instance.specificPreferences
                        or "allocationProjectNumber" in instance.specificPreferences
                    ):
...
class GroupResourceProfileSerializer(proto_utils.create_serializer_class(GroupResourceProfile)):
    url = FullyEncodedHyperlinkedIdentityField(
    ...
    def create(self, validated_data):
        ...
        if compute_prefs:
            all_thrift_instances = all(isinstance(item, GroupComputeResourcePreference) for item in compute_prefs)
    ...
    def update(self, instance, validated_data):
                        ...
                        if "specificPreferences" in incoming_pref and isinstance(
                            ...
                            if "slurm" in incoming_pref["specificPreferences"]:
                                ...
                                if isinstance(incoming_slurm, dict):
                                    existing_slurm = None
                                    if (
                                        hasattr(existing_pref, "specificPreferences")
                                        and existing_pref.specificPreferences
                                        and hasattr(existing_pref.specificPreferences, "slurm")
                                        and existing_pref.specificPreferences.slurm
                                    ):
                ...
                if isinstance(incoming_bq_policy, dict):
                    ...
                    if compute_resource_id and queuename:
                        ...
                        if key in existing_bq_policies_by_key:
                            ...
                            if (
                                "resourcePolicyId" not in incoming_bq_policy
                                or incoming_bq_policy.get("resourcePolicyId") is None
                            ):
        ...
        for compute_resource_preference in instance.computePreferences:
            ...
            if not existing_compute_resource_preference:
                result._removed_compute_resource_preferences.append(compute_resource_preference)
        ...
        for compute_resource_policy in instance.computeResourcePolicies:
            ...
            if not existing_compute_resource_policy:
                result._removed_compute_resource_policies.append(compute_resource_policy)
        ...
        for batch_queue_resource_policy in instance.batchQueueResourcePolicies:
            ...
            if not existing_batch_queue_resource_policy_for_update:
                result._removed_batch_queue_resource_policies.append(batch_queue_resource_policy)
        return result
    ...
    def get_userHasWriteAccess(self, groupResourceProfile):
        ...
        # GroupResourceProfile
        tokens = set(
            [groupResourceProfile.defaultCredentialStoreToken]
            + [cp.resourceSpecificCredentialStoreToken for cp in groupResourceProfile.computePreferences]
        )
...
class SharedEntitySerializer(serializers.Serializer):
    ...
    def update(self, instance, validated_data):
        ...
        existing_user_permissions = {
            user["user"].airavataInternalUserId: user["permissionType"] for user in instance["userPermissions"]
        }
        new_user_permissions = {
            user["user"]["airavataInternalUserId"]: user["permissionType"] for user in validated_data["userPermissions"]
        }
        ...
        (
            ...
            user_revoke_manage_sharing_permission,
        ) = self._compute_all_revokes_and_grants(existing_user_permissions, new_user_permissions)

        existing_group_permissions = {
            group["group"].id: group["permissionType"] for group in instance["groupPermissions"]
        }
        new_group_permissions = {
            group["group"]["id"]: group["permissionType"] for group in validated_data["groupPermissions"]
        }
        ...
        (
            ...
            group_revoke_manage_sharing_permission,
        ) = self._compute_all_revokes_and_grants(existing_group_permissions, new_group_permissions)
        ...
        instance["userPermissions"] = [
            {"user": UserProfile(**data["user"]), "permissionType": data["permissionType"]}
            for data in validated_data.get("userPermissions", instance["userPermissions"])
        ]
        instance["groupPermissions"] = [
            {"group": GroupModel(**data["group"]), "permissionType": data["permissionType"]}
            for data in validated_data.get("groupPermissions", instance["groupPermissions"])
        ]
        return instance

    def _compute_all_revokes_and_grants(self, existing_permissions, new_permissions):
...
class CredentialSummarySerializer(proto_utils.create_serializer_class(CredentialSummary)):
...
class GatewayResourceProfileSerializer(proto_utils.create_serializer_class(GatewayResourceProfile)):
...
class ParserSerializer(proto_utils.create_serializer_class(Parser)):
    url = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:parser-detail", lookup_field="id", lookup_url_kwarg="parser_id"
    )
...
class UserStorageDirectorySerializer(UserHasWriteAccessToPathSerializer):
    ...
    hidden = serializers.BooleanField()
    url = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:user-storage-items", lookup_field="path", lookup_url_kwarg="path"
    )
    ...
    def get_url(self, dir):
        ...
        return request.build_absolute_uri(
            reverse(
                ...
                kwargs={"experiment_id": dir["experiment_id"], "path": dir["path"]},
            )
        )
...
class WorkspacePreferencesSerializer(serializers.ModelSerializer):
    application_preferences = ApplicationPreferencesSerializer(source="applicationpreferences_set", many=True)
...
class IAMUserProfile(serializers.Serializer):
    ...
    url = FullyEncodedHyperlinkedIdentityField(
        view_name="django_airavata_api:iam-user-profile-detail", lookup_field="userId", lookup_url_kwarg="user_id"
    )
    userHasWriteAccess = serializers.SerializerMethodField()
    ...
    def update(self, instance, validated_data):
        ...
        instance["_added_group_ids"] = list(set(new_group_ids) - set(existing_group_ids))
        instance["_removed_group_ids"] = list(set(existing_group_ids) - set(new_group_ids))
        return instance
        ...
        if "showInDashboard" in request.data:
            ...
            if len(existing_entries) > 0:
                ...
                models.NotificationExtension.objects.create(
                    notification_id=notification.notificationId, showInDashboard=request.data["showInDashboard"]
                )


class ExperimentStatisticsSerializer(proto_utils.create_serializer_class(ExperimentStatistics)):
