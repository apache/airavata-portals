...
class APIResultPagination(pagination.LimitOffsetPagination):
    ...
    def paginate_queryset(self, queryset: "APIResultIterator", request: Request, view: Any = None) -> list[Any] | None:
...
def is_shared_dir(path: str) -> bool:
...
class BaseSharedDirPermission(permissions.BasePermission):
    def get_path(self, request: Request, view: Any) -> str:
        raise NotImplementedError()
