...
def custom_exception_handler(exc, context):
    ...
    if isinstance(exc, UnicodeEncodeError):
        ...
        if fse != "utf-8":
            log.error(
                f"filesystem encoding is {fse}, not 'utf-8'. File paths with Unicode characters will produce errors."
            )
...
def generic_json_exception_response(exc, status=status.HTTP_500_INTERNAL_SERVER_ERROR):
