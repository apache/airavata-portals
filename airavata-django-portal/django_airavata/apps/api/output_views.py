...
class DefaultViewProvider:
    ...
    def generate_data(
        self, request: Any, experiment_output: Any, experiment: Any, output_file: Any = None, **kwargs: Any
    ) -> dict[str, Any]:
        return {}
...
class ParameterizedNotebookViewProvider:
    ...
    def generate_data(
        ...
        pm.execute_notebook(
            ...
            "/tmp/output.ipynb",
            parameters=dict(experiment_output={}, experiment={}, output_file=output_file_path, output_dir=output_dir),
        )
...
def get_output_views(
    request: Any, experiment: Any, application_interface: Any = None
        ...
        for output_view_provider_id in output_view_provider_ids:
            ...
            if output_view_provider is not None:
                ...
                if getattr(output_view_provider, "immediate", False):
                    ...
                    data = _generate_data(request, output_view_provider, output, experiment)
...
def _get_output_view_providers(experiment_output: Any, application_interface: Any) -> list[str]:
    ...
    if experiment_output.metaData:
        try:
            ...
            logger.exception(f"Failed to parse metadata for output {experiment_output.name}")
...
def generate_data(
    ...
    **kwargs: Any,
    ...
    experiment_output = [o for o in experiment.experimentOutputs if o.name == experiment_output_name]
...
def _generate_data(
    ...
    **kwargs: Any,
    ...
    if test_mode and settings.DEBUG:
        ...
        and experiment_output.type in (DataType.URI, DataType.URI_COLLECTION, DataType.STDOUT, DataType.STDERR)
        and experiment_output.value.startswith("airavata-dp")
    ):
        data_product_uris = experiment_output.value.split(",")
        data_products = map(lambda dpid: request.airavata_client.research.get_data_product(dpid), data_product_uris)
    ...
    data = generate_data_func(
        request, experiment_output, experiment, output_file=output_files[0] if len(output_files) > 0 else None, **kwargs
    )
...
def _convert_options(data: dict[str, Any]) -> None:
    ...
    for param in data["interactive"]:
        ...
        elif "options" in param and isinstance(param["options"][0], collections.Sequence):
...
def _convert_params_to_type(output_view_provider: Any, params: dict[str, Any]) -> dict[str, Any]:
    ...
    for k, v in params.items():
        ...
        if meta_type == "boolean" or isinstance(default_value, bool):
            ...
            logger.warning(f"Unrecognized type for parameter {k}: meta_type={meta_type}, default_value={default_value}")
    return params
