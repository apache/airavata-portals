...
class ImageBlock(StructBlock):
    ...
    redirect_url = TextBlock(
        required=False, null=True, blank=True, help_text="Give a redirect link on clicking the image"
    )
...
class FontAwesomeIcon(StructBlock):
    ...
    icon_size = IntegerBlock(
        required=False, default=2, null=True, blank=True, help_text="Provide a icon size in number type"
    )
    custom_class = TextBlock(
        required=False,
        blank=True,
...
class HeadingBlock(StructBlock):
    ...
    size = ChoiceBlock(
        choices=[("", "Select a header size"), ("h1", "H1"), ("h2", "H2"), ("h3", "H3"), ("h4", "H4")],
        blank=True,
        required=False,
    )
    custom_class = TextBlock(
...
class CustomEmbedBlock(StructBlock):
    ...
    class Meta:
        ...
        help_text = "Insert a youtube URL e.g https://www.youtube.com/watch?v=SGJFWirQ3ks"
...
class BootstrapButtonMore(StructBlock):
    ...
    body_inline_style = TextBlock(required=False, blank=True, help_text="apply inline CSS styles to body")
    body = RichTextBlock()
...
class BootstrapCard(StructBlock):
    ...
    is_card_img_overlay = BooleanBlock(
        required=False, default=False, help_text="Use image as background for card", label="Image Overlay?"
    )
...
class BootstrapMediaObject(StructBlock):
    ...
    media_img_width = IntegerBlock(
        required=False, blank=True, help_text="Enter an image width as an integer value. Eg(50)"
    )
    media_img_height = IntegerBlock(
        required=False, blank=True, help_text="Enter an image height as an integer value Eg(50)"
    )
...
class ContainerBlock(StructBlock):
    inline_styles = TextBlock(required=False, blank=True, help_text="Apply inline CSS styles to container wrapper.")
    custom_classes = CharBlock(
        required=False,
        help_text="Apply custom CSS classes to container wrapper. You can define CSS classes in a Custom CSS snippet.",
    )
    background_image = ImageChooserBlock(required=False, help_text="Apply background image to container wrapper.")
...
class NavItem(StructBlock):
    ...
    include_in_main_menu = ChoiceBlock(
        choices=[
