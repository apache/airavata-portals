...
@override_settings(ALLOWED_HOSTS=["example.com"], PORTAL_TITLE="My Portal Title")
class SetWagtailSiteTestCase(TestCase):
    ...
    def test_with_localhost_default_site(self):
        ...
        self.assertTrue(Site.objects.filter(hostname="localhost", is_default_site=True).exists())
        call_command("set_wagtail_site")
        # old default should have been deleted
        self.assertFalse(Site.objects.filter(hostname="localhost", is_default_site=True).exists())
        site = Site.objects.get(hostname="example.com")
        self.assertEqual(site.site_name, "My Portal Title")
        self.assertEqual(site.root_page.pk, 3)
        self.assertTrue(site.is_default_site)
    ...
    def test_with_example_com_default_site(self):
        ...
        Site.objects.create(
            hostname="example.com", is_default_site=True, site_name=settings.PORTAL_TITLE, root_page=site_root
        )
        call_command("set_wagtail_site")
        # command shouldn't have created a duplicate
        self.assertEqual(1, Site.objects.filter(hostname="example.com", is_default_site=True).count())
    ...
    def test_with_no_airavata_root_page(self):
        ...
        try:
            ...
            pass
        self.assertTrue(
            Site.objects.filter(hostname="localhost", is_default_site=True).exists(),
